...
PROFILE_DIR="$ROOT/playwright-profiles/${ACCOUNT_ID}"
...
echo "  - usage entry in account_usage.json (if present)"
echo
...
if [[ -f "$ROOT/account_usage.json" ]]; then
  "$ROOT/venv/bin/python3" - "$ACCOUNT_ID" "$ROOT/account_usage.json" <<'PY'
import json
import sys
